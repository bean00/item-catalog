...
def gconnect():
    ...
    # - Create a new user if email doesn't match any existing user's emails
...
def editCategory(category_id):
    ...
    if login_session['user_id'] != category.user_id:
        message = """
        You are not the user who created this category.
        Please logout and then login as that user.
        """
        flash(message)
        return redirect('/')
    name = category.name
...
def editItem(category_id, item_id):
    ...
    if login_session['user_id'] != item.user_id:
        message = """
        You are not the user who created this item.
        Please logout and then login as that user.
        """
        flash(message)
        return redirect('/')
